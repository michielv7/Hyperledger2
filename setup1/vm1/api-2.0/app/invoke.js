...
const invokeTransaction = async (channelName, chaincodeName, fcn, args, username, org_name, transientData) => {
    try {
        ...
        let result
        let message;
        if (fcn === "createRoom" || fcn === "createPrivateRoomImplicitForOrg1"
            || fcn == "createPrivateRoomImplicitForOrg2") {
            result = await contract.submitTransaction(fcn, args[0], args[1], args[2], args[3], args[4]);
            message = `Successfully added the room asset with key ${args[0]}`

        } else if (fcn === "changeRoomOwner") {
            result = await contract.submitTransaction(fcn, args[0], args[1]);
            message = `Successfully changed room owner with key ${args[0]}`
        } else if (fcn == "createPrivateRoom" || fcn == "updatePrivateData") {
            console.log(`Transient data is : ${transientData}`)
            let roomData = JSON.parse(transientData)
            console.log(`room data is : ${JSON.stringify(roomData)}`)
            let key = Object.keys(roomData)[0]
            const transientDataBuffer = {}
            transientDataBuffer[key] = Buffer.from(JSON.stringify(roomData.room))
            ...
            return `Invocation require either createRoom or changeRoomOwner as function but got ${fcn}`
        }
